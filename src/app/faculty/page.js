...
export default function FacultyPage() {
  ...
  // ⭐ Set default program on first load (from URL)
  useEffect(() => {
    if (programFromURL) {
  ...
  const filteredFaculty = useMemo(() => {
    return facultyData
      .filter(f => {
        ...
        // ⭐ Program filter (matches from URL or UI)
        const matchesProgram =
          selectedProgram === "all" ||
          (f.courses || []).some(c => c.program === selectedProgram);

        const matchesCourse =
          selectedCourse === "all" ||
          (f.courses || []).some(c => c.id === selectedCourse);
  ...
  const nonTeachingStaff = filteredFaculty.filter(f =>
    ['administrative', 'support', 'non-teaching'].includes(f.category)
  );
          ...
          <div className="mt-8">
            <div className="flex flex-wrap items-center justify-center gap-4 mb-8">
              ...
              >
                ...
                <span>Staff</span>
                <span className="text-sm font-medium text-gray-500">
                  ({nonTeachingStaff.length + boardMembers.length})
                </span>
              </div>
            </div>
          </div>
